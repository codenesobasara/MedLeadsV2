...
dailyChart(reps: VendorAnalyticsObject['reps'], booth: VendorAnalyticsObject['booth'], selectedDay: string) {
  const series: ApexAxisChartSeries = [];
  const date = new Date(selectedDay);
  const day = String(date.getDate()).padStart(2, '0');
  const month = String(date.getMonth() + 1).padStart(2, '0');
  const year = date.getFullYear();
  const formattedKey = `${day}-${month}-${year}`;
...
totalsChart(reps: VendorAnalyticsObject['reps'], booth: VendorAnalyticsObject['booth']) {
  const series: ApexAxisChartSeries = [];
...
series = computed(() => {
  ...
  const analytics = this.vendorData.vendorAnalytics();
  const reps = analytics?.reps ?? [];
  const booth = analytics?.booth; 
...
setDay(day: string | null) {
 ...
 selectedShiftDate = signal<string| null>(null)
  ...
  const reps = this.vendorData.vendorAnalytics()?.reps?? []
  console.log(`AMOUNT OF REPS ${reps.length}`);
 const hasAnyShifts = reps.some(rep => rep.shifts && rep.shifts.length > 0);  
  reps.forEach(r=>{
      if (!r.shifts?.length) {
    return;
  }
    ...
    repsShifts.forEach(shift=>{
      shifts.push({firstName:r.firstName, lastName:r.lastName,start:shift.startTime, end:shift.endTime})
    })
